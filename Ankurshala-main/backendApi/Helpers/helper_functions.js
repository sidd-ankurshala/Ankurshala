...
module.exports = {
  ...
  uploadMultipleImagesAndVideos: multer({ storage: store }).fields([
    { name: "images" },
    { name: "videos" },
  ]),
  ...
  postToWallet: (servicePath, data) => {
    // console.log("servicePath>>>:", servicePath);
    // console.log("data>>>:", data);
    // console.log("WALLET_URI:", process.env.WALLET_URI);
    // return;
    const config = {
      method: "POST",
      url: `${process.env.WALLET_URI}/${servicePath}`,
      headers: {
  ...
  getFromWallet: (servicePath) => {
    // console.log(
    //   "wallet list>>>>>1112",
    //   `${process.env.WALLET_URI}/${servicePath}`
    // );
    // return;
    const config = {
      method: "GET",
